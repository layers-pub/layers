...
fn main() -> Result<()> {
    ...
    let mut samples: Vec<Sample> = serde_json::from_str(
    ...
    // Synthetic records that exercise the v0.8.0 -> v0.9.0 clean break
    // (annotationMetadata.digest -> contentDigest) and the additive catalogRef
    // field, which the exported layers-repo samples do not carry.
    samples.push(Sample {
        collection: "pub.layers.expression.expression".to_string(),
        origin: "synthetic contentDigest".to_string(),
        record: serde_json::json!({
            ...
            "metadata": {
                "tool": "verify",
                "contentDigest": { "algorithm": "sha256", "value": "deadbeefcafe" }
            }
        }),
    });
    samples.push(Sample {
        collection: "pub.layers.eprint.dataLink".to_string(),
        origin: "synthetic catalogRef".to_string(),
        record: serde_json::json!({
            "$type": "pub.layers.eprint.dataLink",
            "eprintUri": "at://did:plc:example/pub.layers.eprint.eprint/abc",
            "dataKind": "corpus",
            "createdAt": "1970-01-01T00:00:00+00:00",
            "catalogRef": "at://did:plc:example/pub.layers.catalog.collection/xyz"
        }),
    });
    ...
    for sample in &samples {
        ...
        passed += 1;
        println!("  PASS get-put/put-get: {} [{}]", sample.collection, sample.origin);
    }
    println!(
...
fn round_trip(sample: &Sample, schemas: &[Schema]) -> Result<()> {
    ...
    for index in (1..schemas.len()).rev() {
        ...
        // PutGet law: get(put(view, complement)) == view. Restore the source
        // from this view+complement, re-project it, and require the same view.
        let restored_source = put(&lens, &view, &complement)
            .with_context(|| format!("put-get restore {}", VERSIONS[index]))?;
        let (review, _) = project_record(&lens, &restored_source)
            .with_context(|| format!("put-get reproject {}", VERSIONS[index]))?;
        if !instances_equivalent(&view, &review) {
            bail!(
                "put-get law failed at {} -> {}",
                VERSIONS[index],
                VERSIONS[index - 1]
            );
        }
