...
describe('CreateOntologyContent', () => {
  ...
  it('renders the ontology creation form', () => {
    ...
    expect(
      screen.getByText('Create Ontology', { selector: '[data-slot="card-title"]' }),
    ).toBeInTheDocument();
  ...
  it('shows error toast when not authenticated', async () => {
    ...
    await waitFor(() => {
      expect(mockToast.error).toHaveBeenCalledWith('You must be logged in to create an ontology.');
    });
