...
All integration follows the same pattern: AT-URI cross-referencing. A Layers record stores the AT-URI of an external record in a ref field (`sourceRef`, `eprintRefs`, `expressionRef`, etc.), and the Layers appview indexes these references for discovery.
...
- `expression.eprintRefs`: find all Layers data linked to a given eprint
