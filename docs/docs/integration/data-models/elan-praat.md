...
|---|---|---|
| **Annotation Document** (`.eaf` file) | `pub.layers.expression.expression` + `pub.layers.media.media` + annotation layers | The EAF file is a collection of tiers over a media file. In Layers, the media file is a `media` record, the document context is an `expression`, and each tier is an `annotationLayer`. |
...
| License | `pub.layers.corpus.corpus.licensing` (`pub.layers.defs#licensing`, at corpus level) | Licensing terms. The SPDX-based `licensing`/`licenseRef` model supports single, dual/multi (choose-one), composite, and component-scoped licensing. See [CSL-JSON, BibTeX, and DataCite](./csl-datacite). |
...
|---|---|---|
| **TextGrid** (file) | `pub.layers.expression.expression` + `pub.layers.media.media` + annotation layers | A TextGrid is a collection of tiers over a sound file. |
...
| **Interval** | `pub.layers.annotation.defs#annotation` with `anchor.temporalSpan` | `xmin`/`xmax` → `temporalSpan.start`/`temporalSpan.ending` (converted from seconds to milliseconds). `text` → `annotation.value`. |
| **Point** | `pub.layers.annotation.defs#annotation` with `anchor.temporalSpan` | `time` → `temporalSpan.start` (with `ending` = `start`). `mark` → `annotation.value`. |
...
| Word tier | `annotationLayer(kind="tier")` + `pub.layers.expression.expression` tokenization | Word-level intervals can also populate a tokenization. |
...
The word-to-morpheme correspondence uses `pub.layers.alignment.alignment(kind="interlinear", subkind="word-to-morpheme")`.
