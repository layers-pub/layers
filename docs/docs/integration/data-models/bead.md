...
| Per-region reading measures | `pub.layers.judgment.defs#judgment.regionResponses` | Per-region reading times and eye-movement measures (first-fixation, gaze duration, go-past, total time, regression counts, fixation count) for self-paced reading, eye-tracking-while-reading, and maze tasks, as one typed `regionResponse` per region. |
