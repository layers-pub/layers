...
|---|---|---|
| `UDSCorpus` | `pub.layers.resource.collection` + `pub.layers.expression.expression` records | A UDS corpus is a versioned collection of sentence analyzes. The collection record holds corpus metadata (name, version, and `licensing` via `pub.layers.defs#licensing`); each sentence's source text is an expression. The UDS release citation (White et al. 2020) is carried on a linked `pub.layers.eprint.eprint` record's structured `citation`; see [CSL-JSON, BibTeX, and DataCite](./csl-datacite). |
...
1. Create a `pub.layers.resource.collection` for the corpus with version and `licensing` metadata (`pub.layers.defs#licensing`)
