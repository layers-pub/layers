# Decomp / UDS (Universal Decompositional Semantics)
...
|---|---|---|
| `UDSCorpus` | `pub.layers.resource#collection` + `pub.layers.expression` records | A UDS corpus is a versioned collection of sentence analyses. The collection record holds corpus metadata (name, version, license); each sentence's source text is an expression. |
| Document (within corpus) | `pub.layers.expression` | Source text for a sentence or document. |
...
| Syntax node (`ewt-train-12-syntax-3`) | `pub.layers.expression` (kind: `token`) | Tokens with position indices, surface forms, lemmas. |
...
| Argument nodes | `pub.layers.annotation#annotation` referenced via `argumentRef` | Arguments linked to their predicate annotations via `argumentRef` with role information. |
...
| Performative nodes (speaker, addressee, utterance root) | `pub.layers.persona` + annotation with `features.performative = true` | The automatically added speaker and addressee nodes map to persona records. The root performative predicate is an annotation with a feature marking it as performative. |
...
| Decomp/UDS Concept | Layers Equivalent | Notes |
|---|---|---|
| Instance edges (head) | `pub.layers.alignment` with `kind="layer-to-layer"` | Each semantic node's primary link to its syntactic head token. The alignment record maps between the dependency annotation layer and the predicate-argument layer. |
...
| Raw annotations (per-annotator) | `pub.layers.judgment#judgmentSet` + `pub.layers.judgment#judgment` | Individual annotator responses. Each annotator's scalar rating for a property is a `judgment` record with `scalarValue` and `confidence`. |
...
2. For each sentence, create a `pub.layers.expression` record with the source text
3. Create a `pub.layers.segmentation` record with the tokenization from the UD parse
...
6. Create `pub.layers.alignment` records linking UD tokens to semantic nodes (interface edges)
