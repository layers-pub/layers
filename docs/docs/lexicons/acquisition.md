...
The acquisition namespace models the events that produce data and the people they capture: a `session` is a synchronized recording event with one or more streams, and a `participant` is a pseudonymous person or animal recorded in it. Together they give Layers a type level (`judgment.experimentDef`, a protocol) and a token level (a `session`, the event that ran it), and they carry the acquisition metadata that BIDS keeps in `participants.tsv`, `sessions.tsv`, and the electrophysiology sidecars, and that NWB keeps on `Subject` and the session object.
