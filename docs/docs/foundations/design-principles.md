...
The 0.9.0 anchors carry an optional `mediaScope` naming the medium, session clock, stream, and track the coordinates are measured against. Absent scope means the single medium reachable from the annotated expression, so a text or single-track record reads exactly as it did before; present scope is what lets an annotation address one stream of a multi-stream recording without ambiguity.
...
As of 0.9.0 every record `main` declares `key: any` rather than `key: tid` (Scheme A). Record keys may be arbitrary strings, so a publisher can give a record a stable, human-meaningful rkey rather than a time-ordered TID. This widens rkey acceptance and is fully backward-compatible: existing TID-keyed records remain valid. It pairs with the catalog namespace, whose `collection.localId` is a stable, immutable, human-readable identifier that survives handle changes and in-place repo rebuilds, the two events that move an AT-URI's CID and dangle any pin taken against it. Browsable, nestable, citable groupings of records now live in [`pub.layers.catalog`](../lexicons/catalog.md); there is no central index namespace, and a collection's containment spine is child-held (`parentRef`) so no PDS is ever asked to write a link it does not control.
