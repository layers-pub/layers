...
Layers is in active development and accepting comments and discussion. File issues or participate on GitHub: https://github.com/layers-pub/layers
...
flowchart TD
    ...
    EP["<b>Eprint</b><br/><i>structured citations,<br/>publication links</i>"]
...
**Integration layers** (green): Graphs provide typed property graphs for knowledge representation. Media stores raw signals (audio, video, EEG, fMRI). Eprint connects to scholarly metadata through structured citations (CSL-JSON/DataCite fields, authors grounded via ORCID/ROR/OpenAlex); most produces link their papers through an always-array `eprintRefs` field and carry shared structured `licensing` (single, dual, composite, or component-scoped terms via an SPDX expression). Persona tracks annotator frameworks and tool attribution.
